/*
This file is written in ES5 since it's not transpiled by Babel.
This file does the following:
1. Sets Node environment variable.
2. Registers babel for transpiling our code for testing.
3. Disables Webpack-specific features that Mocha doesn't understand.
4. Requires jsdom so we can test via an in-memory DOM in Node.
5. Sets up global vars that mimic a browser.
*/

/* eslint-disable no-var */

/*
Core Section 1:
So that dev specific features such as hot-reloading was disabled when we running test.
This setting assures the .babelrc dev config (which includes hot module reloading code) doesn't
apply for tests. But also, we don't want to set it to production here for two reasons:
1. You won't see any PropType validation warnings when code is running in prod mode.
2. Tests will not display detailed error messages when running against production version code
*/
process.env.NODE_ENV = 'test';

/*
Core Section 2:
Register babel so that it will transpile ES6 to ES5 before our tests run.
*/
require('babel-register')();

/*
Core Section 3:
Disable webpack-specific features for tests since Mocha doesn't know what to do with them.
 */
require.extensions['.css'] = function () {return null;};
require.extensions['.png'] = function () {return null;};
require.extensions['.jpg'] = function () {return null;};

/*
Core Section 4:
Configure JSDOM (virtual memory DOM).
*/
var jsdom = require('jsdom').jsdom;

/*
Core Section 5:
Set a few global variables that help to simulate a browser environment for tests.
The few global variables: document, navigator, window.
*/
var exposedProperties = [ 'document', 'navigator', 'window' ];
global.document = jsdom('');
global.navigator = { userAgent: 'node.js' };
global.window = document.defaultView;
Object.keys(document.defaultView).forEach((property) => {
  if (typeof global[property] === 'undefined') {
    exposedProperties.push(property);
    global[property] = document.defaultView[property];
  }
});
documentRef = document;  //eslint-disable-line no-undef
